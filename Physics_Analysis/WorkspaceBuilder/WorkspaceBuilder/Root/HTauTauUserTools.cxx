#include <WorkspaceBuilder/HTauTauUserTools.h>
#include <fstream>
#include <iostream>
#include <set>
#include <TCanvas.h>
...
namespace WorkspaceBuilder{
  ...
  //-------------------------------------------------------------------------------
  HTauTauUserTools::HTauTauUserTools(std::string statanalysisname, std::string outputpath, std::string version, std::vector<std::string> rootpaths, std::vector<std::string> normfactortxtfilenames, std::vector<std::string> overallsystematicfilenames, std::map<std::string,std::string> parameters, bool unblind, bool info): UserTools(rootpaths, parameters, unblind, info){
  ...
  //-------------------------------------------------------------------------------
  HTauTauUserTools::~HTauTauUserTools(void){
    TString dircmd="mkdir -p ";
    dircmd+=m_outputpath;
    dircmd+="/";
    dircmd+=m_version;
  ...
  //-------------------------------------------------------------------------------
  void HTauTauUserTools::fillUsedHistogramsMap(TDirectory* dir, TString dirname){
    //loop over files
  ...
  //-------------------------------------------------------------------------------
  TH1F* HTauTauUserTools::getHistogram(std::string channel, std::string sample, std::string systematic, int up){
    for(unsigned int ifile=0; ifile<m_rootfiles.size(); ifile++){
    ...
    if(hist!=0){
        ...
        bool isgghxx = TString(sample).Contains("ggH");
        bool isvbfxx = TString(sample).Contains("VBF");
	      if( isgghxx || isvbfxx ){
          srand(m_rndmseed);
  ...
  double HTauTauUserTools::getOverallSys(std::string channel, std::string sample, std::string systematic, int up){
  ...
  double HTauTauUserTools::getLumiInInvPb(std::string channel){
    for(unsigned int ifile=0; ifile<m_rootfiles.size(); ifile++){
      m_rootfiles.at(ifile)->cd();
      TDirectory* channeldir=m_rootfiles.at(ifile)->GetDirectory(channel.c_str());
      if(channeldir!=0){
  ...
  //-------------------------------------------------------------------------------
  std::string HTauTauUserTools::description(){
  ...
  //-------------------------------------------------------------------------------
  double HTauTauUserTools::getNormFactor(std::string normfactor, int up){
  ...
  //-------------------------------------------------------------------------------
  bool HTauTauUserTools::isNuisanceParameterConstant(TString npname){
  ...
  //-------------------------------------------------------------------------------
  TString HTauTauUserTools::correlatedSystematicName(TString name){
  ...
  //-------------------------------------------------------------------------------
  bool HTauTauUserTools::smoothHistogram(TH1F* input, TH1F* nominal, std::string channel, std::string sample, std::string systematic, int up){
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    //TH1F* input is assumed to be the original systematic variation (i.e. input shouldn't be normalised to the nominal histogram)
    ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    if(m_parameters.find("NoSmoothing")==m_parameters.end()){
      //std::cout << "Smoothing" << std::endl;
      if( TString(systematic).Contains("Madgraph") ){
        //std::cout << "Not smoothing MGvsSh!" << std::endl;
        return true;
      }

      double before=input->GetSumOfWeights();
      double after=0.; //Will be set after smoothing
      if(before==0.){
        std::cout<<"INFO: Cannot smoothing histogram for "<<channel<<", "<<sample<<", "<<systematic<<", "<<up<<". Integral is zero."<<std::endl;
        return true;
      }

      if(up!=0 && systematic!="nominal" && nominal!=0){
        //TH1F* bck=(TH1F*)input->Clone();//
        //bck->SetDirectory(0);//
        if(input->GetNbinsX()>=5){ // && TString(channel).Contains("regsig")){ //Smoothing is only supported for histograms with enough number of bins.
          ...
          after=input->GetSumOfWeights();

          if(fabs((after-before)/before)>0.05){
            std::cout<<"WARNING: Smoothing changed the integral more than 5% for "<<channel<<", "<<sample<<", "<<systematic<<", "<<up
                     <<" before:"<<before<<" after:"<<after<<", (after-before)/before="<<(after-before)/before<<std::endl;
          }
          ...
          /*   bool debug=true;
               ...
               ratio->Divide(input);
               ratio->Draw();
               TString filename="debugsmoothing_";
               filename+=channel;
               filename+="_";
               filename+=sample;
               filename+="_";
    ...
    //else{
    //std::cout << "No smoothing done..." << std::endl;
    //}
    return true;
  }

  //-------------------------------------------------------------------------------
  bool HTauTauUserTools::kyleFixHistogram(TH1F* input, std::string channel, std::string sample, std::string systematic, int up, bool issignal){
  ...
  //-------------------------------------------------------------------------------
  int HTauTauUserTools::pruneSystematic(Systematic* systematic, Channel* channel, Sample* sample){
    ...
    //the following is a "fake" pruning implemented to make it possible to run on 7/8 TeV input files, this should not be used for 13 TeV analyses where no prior pruning was applied
    if(m_parameters.find("78TeVPruning")!=m_parameters.end()){

      if(systematic->getUpHistogram()->GetNbinsX()<=3 &&
	 systematic->getDownHistogram()->GetNbinsX()<=3 )
	return 1; //consider as overallsys
      //return 2 if low and high=1
      if(fabs(systematic->getUpNormalisation()-1.)<1E-6 &&
	 fabs(systematic->getDownNormalisation()-1.)<1E-6 )
	return 2;//consider as histosys only
      return 3; // consider as overall and histosys
    }
    ...
    // Pruning for HistSys
    ///////////////////////////////////////////////////////////    
    //Pruning 0: HistSys won't be considered if the original histogram don't have enough statistics.
    double MCuncert=0.;
    ...
    // prune using fixed 1%
    if (doflatprune){
      if (fabs(systematic->getDownNormalisation()-1.) < 0.01 && fabs(systematic->getUpNormalisation()-1.) < 0.01){
    ...
    if (dozttpruning){
      if( TString(channel->name()).Contains("regztt") && !TString(sample->name()).Contains("Zll")){
          if (fabs(systematic->getUpNormalisation()-1.) < 0.2*MCuncert && fabs(systematic->getDownNormalisation()-1.) < 0.2*MCuncert){
              systematic->setSysPrunedBy(PruneUtil::ZttPrune);
              pruneNorm = true;
          }
      }
      if (pruneNorm && TString(channel->name()).Contains("regztt") && TString(sample->name()).Contains("Zll")){
        if (fabs(systematic->getUpNormalisation()-1.) > 0.2*MCuncert && fabs(systematic->getDownNormalisation()-1.) > 0.2*MCuncert){
          systematic->setSysPrunedBy(PruneUtil::ZttPruneRetain);
          pruneNorm = false;
        }
      }
    }
    ...
    if(pruneNorm && !TString(systematic->name()).Contains("Madgraph")) Systematic::removeOverallSys(retvalue);
    ...
    if(systematic->getUpHistogram()->GetNbinsX()<=1) Systematic::removeHistSys(retvalue);
  ...
  //-------------------------------------------------------------------------------
  bool HTauTauUserTools::symmetrizeSystematic(Systematic* systematic, TH1F* nominalhist){
...
//      }
  ...
  //-------------------------------------------------------------------------------
  std::string HTauTauUserTools::getMainParameter(){
  ...
  //-------------------------------------------------------------------------------
  TString HTauTauUserTools::texify(TString input){
    input.ReplaceAll("_","\\_");
    return input;
  }

  //-------------------------------------------------------------------------------
  bool HTauTauUserTools::analyze(std::vector<Channel*> channels){
    TString dircmd="mkdir -p ";
    dircmd+=m_outputpath;
    dircmd+="/";
    dircmd+=m_version;
