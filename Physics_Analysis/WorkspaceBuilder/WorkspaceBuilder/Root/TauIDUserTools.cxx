#include <WorkspaceBuilder/TauIDUserTools.h>
#include <fstream>
#include <iostream>
#include <set>
#include <TCanvas.h>
...
namespace WorkspaceBuilder{
  ...
  //-------------------------------------------------------------------------------
  TauIDUserTools::TauIDUserTools(std::string statanalysisname,
                             ...
                             bool info) : UserTools(rootpaths, parameters, unblind, info)
  {
    ...
    if(m_parameters.size()){
      std::cout<<"INFO: TauIDUserTools::TauIDUserTools() Parameters: \n";
  ...
  //-------------------------------------------------------------------------------
  TauIDUserTools::~TauIDUserTools(void){
    TString dircmd="mkdir -p ";
    dircmd+=m_outputpath;
    dircmd+="/";
    dircmd+=m_version;
    ...
    std::cout<<"INFO: TauIDUserTools::~TauIDUserTools() - File '"+rootfilename+"' is created."<<std::endl;
  ...
  //-------------------------------------------------------------------------------
  void TauIDUserTools::fillUsedHistogramsMap(TDirectory* dir, TString dirname){
    //loop over files
  ...
  //-------------------------------------------------------------------------------
  TH1F* TauIDUserTools::getHistogram(std::string channel, std::string sample, std::string systematic, int up){
    for(unsigned int ifile=0; ifile<m_rootfiles.size(); ifile++){
  ...
  double TauIDUserTools::getOverallSys(std::string channel, std::string sample, std::string systematic, int up){
  ...
  double TauIDUserTools::getLumiInInvPb(std::string channel){
    for(unsigned int ifile=0; ifile<m_rootfiles.size(); ifile++){
      m_rootfiles.at(ifile)->cd();
      TDirectory* channeldir=m_rootfiles.at(ifile)->GetDirectory(channel.c_str());
      if(channeldir!=0){
  ...
  //-------------------------------------------------------------------------------
  std::string TauIDUserTools::description(){
  ...
  //-------------------------------------------------------------------------------
  double TauIDUserTools::getNormFactor(std::string normfactor, int up){
  ...
  //-------------------------------------------------------------------------------
  bool TauIDUserTools::isNuisanceParameterConstant(TString npname){
  ...
  //-------------------------------------------------------------------------------
  TString TauIDUserTools::correlatedSystematicName(TString name){
  ...
  //-------------------------------------------------------------------------------
  bool TauIDUserTools::smoothHistogram(TH1F* input, TH1F* nominal, std::string channel, std::string sample, std::string systematic, int up){

    if(m_parameters.find("NoSmoothing")==m_parameters.end()){
      std::cout << "INFO: TauIDUserTools::smoothHistogram() - No Smoothing will be applied!" << std::endl;
      return true;
    }
  ...
  //-------------------------------------------------------------------------------
  bool TauIDUserTools::kyleFixHistogram(TH1F* input, std::string channel, std::string sample, std::string systematic, int up, bool issignal){
...
*/
  ...
  //-------------------------------------------------------------------------------
  int TauIDUserTools::pruneSystematic(Systematic* systematic, Channel* channel, Sample* sample){
    ...
    if(m_parameters.find("NoPruning")!=m_parameters.end()){
      std::cout<<"INFO: TauIDUserTools::pruneSystematic() - No Pruning will be applied!\n";
      return Systematic::NotPruned;
    }

    //keep NPs in any case
    if(m_parameters.find("KeepNPcontaining")!=m_parameters.end()){
      if (systematic->name().find( m_parameters["KeepNPcontaining"] ) != std::string::npos) {
        std::cout<<"INFO: TauIDUserTools::pruneSystematic() - Pruning won't be applied '"
          +systematic->name()+"'/'"+channel->name()+"'/'"+sample->name()+"' (protected NP)..."<<std::endl;
        return Systematic::NotPruned;
      }
    }
    ...
    if((systematic->getUpNormalisation() < 1E-4 || systematic->getDownNormalisation() < 1E-4) && !(systematic->oneSided())){
        std::cout<<"INFO: TauIDUserTools::pruneSystematic() - Pruning '"
    ...
    if( systematic->hasTag("FourVectorSystematic") ){
      ...
      if(pvalue_chi2up<=threshold && pvalue_chi2down<=threshold){
        std::cout<<"INFO: TauIDUserTools::pruneSystematic() - Pruning '"
          +systematic->name()+"'/'"+channel->name()+"'/'"+sample->name()+"' (HistoSys - chi2)..."<<std::endl;
    ...
    if (doflatprune){
      if (fabs(systematic->getDownNormalisation()-1.) < 0.01 && fabs(systematic->getUpNormalisation()-1.) < 0.01){
        ...
        std::cout<<"INFO: TauIDUserTools::pruneSystematic() - Pruning '"
          +systematic->name()+"'/'"+channel->name()+"'/'"+sample->name()+"' (OverallSys - SameNorm)..."<<std::endl;
  ...
  //-------------------------------------------------------------------------------
  bool TauIDUserTools::symmetrizeSystematic(Systematic* systematic, TH1F* nominalhist){
  ...
  //-------------------------------------------------------------------------------
  std::string TauIDUserTools::getMainParameter(){
  ...
  //-------------------------------------------------------------------------------
  TString TauIDUserTools::texify(TString input){
    input.ReplaceAll("_","\\_");
    return input;
  }

  //-------------------------------------------------------------------------------
  bool TauIDUserTools::analyze(std::vector<Channel*> channels){
    TString dircmd="mkdir -p ";
    dircmd+=m_outputpath;
    dircmd+="/";
    dircmd+=m_version;
