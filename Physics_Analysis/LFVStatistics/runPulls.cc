...
int main(int argc, char* argv[]){
  ...
  //{
    ...
    system(("mkdir -vp " + thisdir + "/" + "root-files/" + string(folder) + "_pulls").c_str());
...
//        }
	...
	*/
	...
	// store result in root file
        ...
        fileName <<  thisdir << "/" << "root-files/" << folder << "_pulls/" << nuipName << ".root";
	...
	for (double nuip_val = nuip_hat - fabs(nuip_errdown)*1.1*2; nuip_val <= nuip_hat + fabs(nuip_errup)*1.1*2; nuip_val += step_size)
	  {
	    ws->loadSnapshot("tmp_snapshot2");
