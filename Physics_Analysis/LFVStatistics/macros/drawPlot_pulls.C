...
int showTopParameters      = 66;    // -1 to show all parameters
...
Int_t style_postfit        = 3353;
Int_t style_postfit_ol     = 3353;
Int_t style_prefit         = 3335;
Int_t style_prefit_ol      = 3335;
...
void drawPlot_pulls(bool _useRelativeImpact = 0, double use_scale_poi=1, double use_scale_theta = 1, string mass = "125", string cardName = "", bool remakeAscii = 0, string overlayCard="") {
  ...
  //  gSystem->AddIncludePath(" -D__USE_XOPEN2K8");
    ...
    if (drawParamNames) pad1->SetLeftMargin(0.37);
	...
	*/
    ...
    npbox->SetTextSize(0.02);
    npbox->SetTextAlign(22);
    npbox->AddText("NP Classification:");
    npbox->AddText(Form("#color[%d]{Statistical}",kMagenta+1));
    npbox->AddText(Form("#color[%d]{MC Statistical}",kAzure+1));
    npbox->AddText(Form("#color[%d]{Systematic}",kGreen+3));
    //    npbox->AddText(Form("#color[%d]{Theoretical: MUTED}",kOrange+10));
...
void drawPlot_pulls2(string cardName, string mass, TCanvas* c1, TPad* pad1, TPad* pad2) {
	...
	if (tmpname.Contains("gamma_stat_") || tmpname.Contains("ATLAS_norm") || tmpname.Contains("ATLAS_shape_")) {
	...
	*/
    ...
    for (int i = offset; i < nrNuis+offset; i++) {
      ...
      int labelcolor = kGreen+3;
      if (tmpstr.Contains("norm_")) labelcolor =  kMagenta+1 ;
      if (tmpstr.Contains("stat_") || tmpstr.Contains("shape_")) labelcolor =  kAzure+1 ;
    ...
    h->SetLabelSize(labelSize>0.024?0.024:labelSize,"Y");
    h->SetLabelSize(0.024,"Y");
    ...
    if (drawPrefitImpactBand) {
        leg->AddEntry(gr_poi_nom, "#splitline{-1#sigma Prefit Impact on #hat{#mu}}{Right/Left #Rightarrow -/+ Correlation}","f");
        if (overlay != "") leg->AddEntry(gr_poi_nom_ol, "Alt -1#sigma Prefit Impact on #hat{#mu}","f");
    }
    if (drawPostfitImpactBand) {
        leg->AddEntry(gr_poi, "#splitline{+1#sigma Postfit Impact on #hat{#mu}}{Left/Right #Rightarrow -/+ Correlation}","f");
