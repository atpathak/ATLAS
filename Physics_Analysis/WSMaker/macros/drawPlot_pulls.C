...
bool drawPrefitImpactBand  = true; //false;  // && (mode != error); // draw a band with the variation of muhat due to each theta (prefit uncertainty)
...
double scale_poi           = 1.85; //1.55(etau_mva); //1.85;  // zoom the impact axis
double scale_theta         = 0.95; //1.55(etau_mva); //1.35;(mtau_all)  // zoom the pull axis; 3.4 for 1lep...1.7 for everybody else
...
int showTopParameters      = 25;    // want to see first 25
...
void drawPlot_pulls(string mass = "125", string cardName = "", bool remakeAscii = 0, string overlayCard="", bool postFitOrder = true) {
  gStyle->SetHatchesLineWidth(hatch_width);
    ...
    if (drawParamNames) pad1->SetLeftMargin(0.55);//Graph size
  ...
  //     double sigma_tot_lo  = 0.;
    ...
    for (int i = 0; i < nrNuis; i++) {
      ...
      cout << "Rank " 
	   << nrNuis - i << ":  \t" 
	   << "Post: "
	   << fabs(poi_down[i]-poi_hat[i]) << "  \t" 
	   << fabs(poi_up[i]-poi_hat[i]) << "  \t"  
	   << (poi_up[i] + poi_down[i])/2 << " \t"
	   << "Pre: "
	   << fabs(poi_nom_down[i]) << "  \t" 
	   << fabs(poi_nom_up[i]) << "  \t"  
	   << (poi_nom_up[i] + poi_nom_down[i])/2 << " \t"
	   << labels[i] << endl;
