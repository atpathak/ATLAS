    ...
    string loglevel = "DEBUG" )
{
    ...
    LOG(logDEBUG) << "Workspace: " << wsName ; // adding by me
    if (!ws) {
    ...
    LOG(logDEBUG) << "ModelConfig: " << modelConfigName ; // adding by me
    if (!mc) {
    ...
    LOG(logDEBUG) << "Dataset: " << dataName ; // adding by me
    if (!data) {
    ...
    cout << "parsed.size()" << parsed.size() << endl; // adding by me
    ...
    cout << "nrNuis size : " << nrNuis << endl; // adding by me
    cout<<"start doing 7"<<endl;
    
    // create nll and do unconditional fit
    // ws->loadSnapshot("ucmles");
    cout << "pois.size()" << pois.size() << endl; // adding by me
    ...
    TString a = "ATLAS_BTAG_C_0";
    cout << " namesys : " << namesys << endl;
    //for (int in = 0; in < nrNuis; in++) {
    for (int in = namesys-1; in < namesys; in++) {
	...
	*/
	...
	//RooRealVar* nuip = (RooRealVar*)nuis->find(vec_nuis[3].c_str());
        string nuipName(nuip->GetName());

        cout<<" doing nuip name "<< nuipName <<endl;
	//cout << " a.Data() : " << a.Data() << endl ;
	//if ( nuipName.find("ATLAS_BTAG_C_0") == a.Data()) {
	  //cout << " entring into If " << endl ;
	  if (variable != NULL && nuipName != string(variable)) continue;
	  //if (variable != NULL && nuipName.find("ATLAS_BTAG_B_1") != string::npos ) continue;
	  //
