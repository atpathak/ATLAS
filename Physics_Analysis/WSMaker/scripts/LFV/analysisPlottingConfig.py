...
class Config(cfg.PlottingConfig):
    ...
    def __init__ (self, options):
        ...
        #LFV_mtau_cba_vbf_signal
        #LFV_mtau_cba_sr3_vbf_signal
        #LFV_mtau_cba_sr2_vbf_signal
        #LFV_mtau_cba_sr1_vbf_signal
        #Htt_yearAll_chanll_catvbf_regtop_selCBA_Prefit_logy
        #Htt_yearAll_chanll_catvbf_regtop_selCBA_Prefit
        #Htt_yearAll_chanll_catvbfloose_regsig_selCBA
        #LFV_yearAll_chanmtau_regsr1vbf_selCBA_signal
        
        # for child classes to use
        # self.loggingLvl = logging.INFO
        self.loggingLvl = logging.DEBUG
        self.verbose = False
        self.formats = [ 'eps', 'pdf', 'png', 'root']
        self.blind = False
        ...
        #self.L = ""
        self.file_tags = ["year","chan","cat","reg","sel","signal"]
        self.weight_tags = []
        self.sig_names = ["VBFH", "ggH","ttH125","WH125","ZH125"]
        ...
        self.bkg_tuple = {
            ...
            "VH" : ("VH", 24, ROOT.kMagenta + 1, []),
            "Zll" : ("Zll", 23, ROOT.kMagenta + 3, []),
            "W" : ("W", 22, ROOT.kMagenta + 2, []),
            "VV" : ("Diboson", 21, ROOT.kMagenta, []),
            "Top" : ("top", 20, ROOT.kBlue + 3, []),
            "Fakes" : ("Fakes", 19, ROOT.kYellow, []),
            "Htt" : ("Htt", 25, ROOT.kGreen, []),
        ...
        "Lepton": [False, ["el_","mu_","tau_","EL_","MU_","TAU_","TES_","EG_"], []],
        ...
        self.suspicious_syst = ["btag_b_0","btag_light_0","MadgraphVsSherpa"]
        self.problematic_pull_thres = 0.5
        self.problematic_constr_thres = 0.75
        self.chanlabels = {
            "lh" : "#tau_{l}#tau_{h}",
            "mtau" : "#mu#tau_{h}",
            "etau" : "e#tau_{h}",
    ...
    def splitTable(self, reg):
        region = reg.split('_cat')[0]
        if "boost" in reg:
            region += "_boost"
        if "vbf" in reg:
            region += "_vbf"
    ...
    def getTableTitle(self, name, mode):
            properties = {}
            blocks = name.split('_')[1:] # remove "Htt_" remove "LFV_"
            tags = self.file_tags
            title = name
            i=0
            val = blocks
...
#            }
        ...
        region = self.chanlabels[props["chan"]] #+ " " + self.catlabels[props["v"]]
        print region
