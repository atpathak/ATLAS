...
from ROOT import RooArgSet, RooArgList, RooAddition, RooAbsData, RooAbsReal, RooHist
...
def getWorkspace(version, mass):
    #wsf = os.path.join("output", str(version), "combined", str(mass))
...
def getComponents(cfg, w):
    ...
    for tt in categories(channelCat):
        ...
        binWidth = pdftmp.getVariables().find("binWidth_obs_x_{0}_0{1}".format(ttname,sure+yup))
        if not binWidth:
...
def plotSumOfCats(cfg, w, rfr, mass, suffix, plotdir, save_hists, list_objs, list_comps, name, weights=None):
...
# TODO: Elisabeth's stuff
def makeWeightedSumPlot(cfg, ws, rfr, mass, suffix, plotdir, save_hists, objs_dict, yields, comps, name, restrict_to=[], exclude=[]):
# def makeWeightedSumPlot(rfr, suffix, plotdir, save_hists, objs_dict, yields, comps, name, restrict_to=[], exclude=[]):
    ...
    plotSumOfCats(cfg, ws, rfr, mass, suffix, plotdir, save_hists, list_tot, comps_tot, name)
    if not "AZh" in os.getenv("ANALYSISTYPE"):
        plotSumOfCats(cfg, ws, rfr, mass, suffix, plotdir, save_hists, list_tot, comps_tot, name+"_Higgsweighted", weights_tot)
    else:
        print( "Skipping 'Higgsweighted' since AZh!")
...
# def makePlotsSums(w, rfr, is_prefit, suffix, plotdir, save_hists = False):
    ...
    plottingFunc = lambda name, rt, ea : makeWeightedSumPlot(cfg, w, rfr, mass,
                                                             suffix, plotdir,
                                                             save_hists,
                                                             objs_dict, yields,
                                                             comps, name = name,
...
def ttname_mod(cfg, ttname):
    if cfg.dataname!='combData':
        return '',''
    elif '2011' in ttname:
        return '_7TeV','_78TeV'
    elif '2012' in ttname:
        return '_8TeV','_78TeV'
    elif '2015' in ttname or '2016' in ttname:
        return '_13TeV',''
    return '',''
...
def components(pdf, ttname, iscomb):
    ...
    if iscomb:
        sure,yup=ttname_mod(ttname)
        # Run1 pattern
        if yup != '':
            modelName1 = '{0}_model{1}_model_{0}{1}_edit{2}_model_{0}{1}_edit{2}_edit'.format(ttname,sure,yup)
        # Run2 pattern
        else:
            modelName1 = '{0}_model{1}_model_{0}{1}_edit'.format(ttname,sure)
    pdfmodel1 = pdf.getComponents().find(modelName1)
    if not pdfmodel1:
        it=pdf.getComponents().createIterator()
        var=it.Next()
        print( '---------ttname is-------{}'.format(ttname),pdfmodel1==0)
        while var:
            nom=var.GetName()
