...
class Setup(object):
    ...
    def finish_initialize(self):
        ...
        if not self.initialized:
            ...
            logging.debug("plot_with_additional_signal = {}".format(self.plot_with_additional_signal()))
            ...
            if self.sig_to_use is not None:
                ...
                if "#mu" not in self.sig_to_use.title and not "AZh" in os.getenv("ANALYSISTYPE"):
                    self.sig_to_use.title += " (#mu={0:.2f})".format(self.sig_to_use.muhat)
    ...
    def make_complete_plot(self, can_name = "", do_ratio = False, logy = False, ytitle = "Events", ybounds=(0.5, 1.5)):
        ...
        if do_ratio:
            do.cd()
            self.draw_dataMC_ratio(True, ybounds, xbounds=xbounds, xtitle="", logx=logx)
            up.cd()
        leg_pos = self.cfg.get_legend_pos_from_properties(self.properties)
        if leg_pos is None: # default
            leg_pos = [0.62, 0.42, 0.865, 0.925]
    ...
    def make_bkg_substr_plot(self, can_name = ""):
        ...
        if self.cfg._weighted:
            return setup_tmp.make_complete_plot(can_name, ytitle="Events / {0:.0f} GeV (Weighted, backgr. sub.)".format(binwidth) )
#            return setup_tmp.make_complete_plot(can_name, ytitle="Weighted events after subtraction")
    ...
    def make_SoB_plot(self, can_name = "",do_ratio = True, Ratioybounds=(0.4, 1.6), ratioType = 0):
        ...
        binwidth = first_h.GetHistogram().GetBinWidth(3)#(first_h.GetHistogram().GetXaxis().GetXmax() - first_h.GetHistogram().GetYaxis().GetXmin()) / (first_h.GetHistogram().GetNbinsX())
        first_h.GetHistogram().GetYaxis().SetTitle("Events / {0:.2f}".format(binwidth))
    ...
    def build_legend(self, coords, logy = False):
        ...
        for b in reversed(self.bkgs):
            if (b.h.Integral() > sumbkgs * self.cfg.thresh_drop_legend) or (logy
                ...
                logging.info("the name is {}".format(b.name))
                if self.properties:
...
#            hdash.GetYaxis().SetNdivisions(503) # init
        ...
        if self.properties is not None:
            ...
            #if "VHbbRun2" in os.getenv("ANALYSISTYPE") or "AZh" in os.getenv("ANALYSISTYPE"):
             #   hdash,l = cfg.postprocess_dataMC_ratio_histogram(self.properties, hdash)
            #else:
            hdash = cfg.postprocess_dataMC_ratio_histogram(self.properties, hdash)

        draw(hdash, "hist")
        #if "VHbbRun2" in os.getenv("ANALYSISTYPE") or "AZh" in os.getenv("ANALYSISTYPE"): draw(l, "")
        #draw(htemp, "pz0")
    ...
    def draw_lumi(self, pos):
        ...
        def draw_line(x, y, s, lumi):
            l.DrawLatex(x, y, "#sqrt{{s}} = {1} TeV, {0} fb^{{-1}} ".format(lumi, s))
...
class Data(object):
    ...
    def __init__(self, cfg, h):
        ...
        if "AZh" in os.getenv("ANALYSISTYPE"):
            ...
            self.title = "Pseudo-Data {0}".format(cfg.get_year_str())
        else:
            self.title = "Data {0}".format(cfg.get_year_str())
...
class Signal(object):
    ...
    def __init__(self, cfg, hist, mass, muhat=None, is_expected=False, is_additional=False):
        ...
        if self.mult_factor > 1 and not "AZh" in os.getenv("ANALYSISTYPE"):
    ...
    def make_additional_signal(self, new_mult_factor, dist, prop):
        ...
        if self.muhat is None:
            return None
        logging.info("Signal with muhat = {}".format(self.muhat))
        ...
        logging.info("Scaled down by muhat = {0}".format(self.muhat))
        newsig.mult_factor /= self.muhat
        newsig.scale()
        logging.info("Post scaled yields = {0}".format(newsig.h.Integral()))
        return newsig
...
def getPropertiesFromTag(cfg, regname):
    ...
    print( 'GETTING PROPERTIES FROM TAG: {}'.format(regname))
    properties = {}
    blocks = regname.split('_')[1 if (regname.find('Region_')==0 or regname.find('Htt_')==0 or regname.find('LFV_')==0) else 0:]  # remove "Region_" # remove "Htt_" # remove "LFV_"
    for b in blocks: # SKC
