...
//    ROOT::Math::MinimizerOptions::SetDefaultStrategy(2);
    ...
    //if(!w->loadSnapshot(snapshotName)) {
      ...
      if( writePostfitAsimData ) {
        RooAbsData* postfit_asimov_data = makeAsimovData(mc, 0, w, mc->GetPdf(), (RooDataSet*)localData, firstPOI->getVal()); // no fit and build asimov with mu=mu_asimov
        TString asimName("asimovData_paramsVals_"+globOrAsim+"Fit");
        if(IsConditionnal) { asimName.Append("_mu"+TString(Form("%4.2f",mu))); }
        else { asimName.Append("_unCond"); }
...
void FitCrossCheckForLimits(const Algs algorithm         = PlotHistosBeforeFit,
                            ...
                            bool createPostfitAsimov    = false) {
