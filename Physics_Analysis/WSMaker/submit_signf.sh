...
#StatAnalysis=LFV_etau_cba
cat > todo_${channel}_signf_exp <<EOF
#!/bin/bash
cd /afs/cern.ch/user/a/atpathak/afswork/public/WSMaker
source setup.sh
#python scripts/getSig.py ${channel}/LFV13TeVCombinationCBA 0 125 2>&1 | tee log/signf/${channel}_signf.log
python scripts/getSig.py data_new/${channel}/v20180326/${StatAnalysis} 1 125 2>&1 | tee log/signf_exp/${channel}_signf_exp.log
EOF
chmod +x todo_${channel}_signf_exp
logfile=log/signf_exp/${channel}.log
rm -f $logfile ; bsub_lxplus 2nw $logfile 0 ./todo_${channel}_signf_exp
done
