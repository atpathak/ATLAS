...
int main(int argc, char* argv[]) {
   ...
   TString inDir   = "root://eosatlas//eos/atlas/atlascerngroupdisk/phys-higgs/HSG5/Run2/ICHEP2016/";
   int arg = 1;
   while (arg < argc) {
     ...
     else if ( strcmp(argv[arg], "-s")     * strcmp(argv[arg], "-subDir")   == 0 ) subDir  = argv[arg+1];
   ...
   if(type == "Run1") {
     ...
     mySplitter.SetSubDir(subDir);
     mySplitter.SplitIt();

     // run njet merge and rename mva28
     ...
     if ( tmp == NULL ) {
       ...
       if ( s.compare("VHbbRun2") == 0 ){
         ...
         cout << "rename mva28 for VHbbRun2 0lep" <<endl;
         system( ("RenameVar.py "+version).Data() );

         cout << "merge 3 and 4+ jet bins for VHbbRun2 2lep" <<endl;
         system( ("Mergenjetregions.py  "+version).Data() );

	 if(version.Contains("CUT")){
	   cout << "WhfCR and SR for Run2 1lep CBA" <<endl;
	   system( ("MergeCR.py  "+version).Data() );
	 }
