...
def main():
  ...
  dist = config.getTagStringDefault('distributionIF','Pt_Mt') 
  binning = config.getTagVDouble('binningIF')
  ...
  outFile = config.getTagStringDefault('outputFileIF','output/dummy/dummy.root')
  ...
  channel = 'etau'
  processesAntiTau = {'Data':'./bkg/'+channel+'/Fake/data',
#                      'W':'./bkg/'+channel+'/Fake/mc',
                      'W':'./bkg/'+channel+'/Fake/mc/[WjetsSh+WjetsEWSh]',
                      'Z':'./bkg/'+channel+'/Fake/mc/[ZllSh+ZllEWSh]',
                      'Ztt':'./bkg/'+channel+'/Fake/mc/[ZttSh+ZttEWSh]',
                      'Top':'./bkg/'+channel+'/Fake/mc/Top',
                      'DiBoson':'./bkg/'+channel+'/Fake/mc/Diboson'
                      }
  
  processesTau = {'Data':'./data/'+channel,
#                  'W':'./bkg/'+channel+'/',
                  'W':'./bkg/'+channel+'/[WjetsSh+WjetsEWSh]',
                  'Z':'./bkg/'+channel+'/[ZllSh+ZllEWSh]',
                  'Ztt':'./bkg/'+channel+'/[ZttSh+ZttEWSh]',
                  'Top':'./bkg/'+channel+'/Top',
                  'DiBoson':'./bkg/'+channel+'/Diboson'
                  }
  ...
  if not binning:
    ...
    dummyHisto = reader.getHistogram('./data','CutPreselection/'+dist)
    ...
    for prong in prongs:
      for category in categories:
        for trigger in triggers:
          name = channel+'_'+str(process)+'_'+str(category)+'_'+str(prong)+'_'+str(trigger)+'_'+str(year)
          histo = TH1D(name,name,nBins,binArray)
          histo.GetXaxis().SetTitle(str(dist))
          histo.GetYaxis().SetTitle('Iso Factor')
...
def calcFakeFactor(reader, processesTau, processesAntiTau, process, prong, category, trigger, lowBin, highBin, dist, binArray):
  ...
  for p in processesTau:
    ...
    if p == 'Data':
      ...
      histogramTauData = reader.getHistogram(path,'Cut'+category+'NoLep'+prong+trigger+controlRegion+'/'+dist)
      ...
      histogramTauNoJet = reader.getHistogram(path,'Cut'+category+'NoLep'+prong+trigger+controlRegion+'/'+dist)
  ...
  for p in processesAntiTau:
    ...
    if p == 'Data':
      ...
      histogramAntiTauData = reader.getHistogram(path,'Cut'+category+'NoLep'+prong+trigger+controlRegion+'/'+dist)
      ...
      histogramAntiTauNoJet = reader.getHistogram(path,'Cut'+category+'NoLep'+prong+trigger+controlRegion+'/'+dist)    
      #name = 'Cut'+category+'NoJet'+prong+trigger+controlRegion+'/'+dist
      #print name
      rebinnedHistogramAntiTauNoJet = histogramAntiTauNoJet.Rebin(len(binArray)-1,'',binArray)
      sumAntiTauNoJet += -1.0*rebinnedHistogramAntiTauNoJet.IntegralAndError(lowBin,highBin,integralAntiTauNoJetErr)
