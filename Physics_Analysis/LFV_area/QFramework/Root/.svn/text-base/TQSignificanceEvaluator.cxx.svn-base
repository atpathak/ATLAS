...
bool TQSignificanceEvaluator::updateHists(std::vector<int> axesToScan, TQGridScanner* scanner,
																					int axisToEvaluate) {
	return true;
}
...
void TQSignificanceEvaluator::bookNF(TString path){
  // book an NF
  this->autoNFs.push_back(path);
}

void TQSignificanceEvaluator::addRegion(TString cutname){
  // add a region (cut)
