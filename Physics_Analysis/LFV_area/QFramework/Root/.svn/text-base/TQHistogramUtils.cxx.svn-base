...
//
// - createHistogram(...) creates and returns a new histogram from a definition string
// using a syntax similar to the constructor of the corresponding
// histogram class 
//
// - getHistogramDefinition(...) ---
//
// - getBinningDefinition(...) ---
//
// - method(...) ---
//
// - method(...) ---
...
TH1 * TQHistogramUtils::getRebinnedFlat(TH1 * histo, int nBins) {
  ...
  /* stop if input histogram is invalid */
...
std::vector<int> TQHistogramUtils::getBinBordersFlat(TH1 * histo, int nBins, bool includeOverflows) {
  ...
  /* done in case only one bin was requested */
...
std::vector<int> TQHistogramUtils::getBinBordersFlatPrivate(TH1 * histo, int nBins, int iFirstBin, double meanPerBin, double &quality, bool includeOverflows) {
...
bool TQHistogramUtils::hasBinLabels(TAxis* a){
  ...
  return a->fLabels && a->fLabels->GetSize()>0;
}
...
bool TQHistogramUtils::addHistogram(TH1 * histo1, TH1 * histo2, double scale, double scaleUncertainty, double corr12, bool includeScaleUncertainty) {
  ...
  if(p1 && p2){
    ...
    int binmin = 0;
    int binmax = nbins+1;
    if(histo1->GetXaxis()->IsAlphanumeric()){
      binmin = 0;
      binmax = nbins-1;
    }
...
TH1 * TQHistogramUtils::getReweightedHistogram(
  ...
  /* stop if input histogram sare invalid */
...
double TQHistogramUtils::getChi2(TH1 * histo1, TH1 * histo2) {
  ...
  /* stop if an invalid histogram was passed */
  if (!histo1 || !histo2)
    return -1.;
...
void TQHistogramUtils::unifyMinMax(TCollection * histograms, double vetoFraction) {
  ...
  /* stop if list of histograms is invalid */
  if (!histograms)
    return;
...
void TQHistogramUtils::unifyMinMax(TH1 * h1, TH1 * h2, TH1 * h3, double vetoFraction) {
  ...
  /* create a list for the histograms */
  TList * histograms = new TList();
...
bool TQHistogramUtils::getMinMaxBin(
  ...
  for (int i = 0; i < nBins; ++i) {
    ...
    if (iBinMin == -1 || lower < min){
      min = lower;
      iBinMin = i;
    }
    if (iBinMax == -1 || upper > max){
      max = upper;
      iBinMax = i;
    }
  }
...
bool TQHistogramUtils::getMinMax(
  ...
  int minBin = -1;
  int maxBin = -1;
  if (getMinMaxBin(histo, minBin, maxBin, includeError, includeUnderflowOverflow)) {
    min = histo->GetBinContent(minBin) - (includeError ? histo->GetBinError(minBin) : 0.);
    max = histo->GetBinContent(maxBin) + (includeError ? histo->GetBinError(maxBin) : 0.);
    return true;
  } else {
    return false;
  }
...
int TQHistogramUtils::getMinBin(TH1 * histo, bool includeError, bool includeUnderflowOverflow) {
  int minBin = -1;
  int maxBin = -1;
  getMinMaxBin(histo, minBin, maxBin, includeError, includeUnderflowOverflow);
  return minBin;
}
...
int TQHistogramUtils::getMaxBin(TH1 * histo, bool includeError, bool includeUnderflowOverflow) {
  int minBin = -1;
  int maxBin = -1;
  getMinMaxBin(histo, minBin, maxBin, includeError, includeUnderflowOverflow);
  return maxBin;
...
double TQHistogramUtils::getMin(TH1 * histo, bool includeError, bool includeUnderflowOverflow) {
  double min = 0.;
  double max = 0.;
  getMinMax(histo, min, max, includeError, includeUnderflowOverflow);
  return min;
}
...
double TQHistogramUtils::getMax(TH1 * histo, bool includeError, bool includeUnderflowOverflow) {
  double min = 0.;
  double max = 0.;
  getMinMax(histo, min, max, includeError, includeUnderflowOverflow);
  return max;
...
bool TQHistogramUtils::extractStyle(TH1 * histo, TQTaggable * tags, const TString& styleScheme) {
  ...
  // stop if either the histo or the taggable is invalid
  if (!histo || !tags) {
    return false;
  }
...
bool TQHistogramUtils::haveEqualShapeRel(TH1 * h1, TH1 * h2, double rel) {
  ...
  /* valid input histograms expected */
  if (!h1 || !h2)
    return false;
...
int TQHistogramUtils::ensureMinimumBinContent(
  ...
  /* valid input histogram expected */
  int dim = getDimension(histo);
  if (dim < 1 || dim > 2) {
    return -1;
  }
...
void TQHistogramUtils::remap(TAxis* ax,double min, double max) {
  // remap the axis of a histogram (without actually changing the binning)
...
double TQHistogramUtils::getMax(TCollection* c, bool includeUnderflowOverflow){
  ...
  while(itr.hasNext()){
    TH1* hist = dynamic_cast<TH1*>(itr.readNext());
    if(!hist) continue;
    max = std::max(TQHistogramUtils::getMax(hist,false,includeUnderflowOverflow),max);
  }
  return max;
}
...
double TQHistogramUtils::getMin(TCollection* c, bool includeUnderflowOverflow){
  ...
  while(itr.hasNext()){
    TH1* hist = dynamic_cast<TH1*>(itr.readNext());
    if(!hist) continue;
    min = std::min(TQHistogramUtils::getMin(hist,false,includeUnderflowOverflow),min);
  }
  return min;
}
...
void TQHistogramUtils::interpolateGraph(TGraph* g, size_t increasePoints){
...
TH2* TQHistogramUtils::convertMatrixToHistogram(TMatrixTT* matrix, const TString& name){
...
namespace TQHistogramUtils {
  template TH2* convertMatrixToHistogram<TMatrixT<float > >(TMatrixT<float >* matrix, const TString& name);
  template TH2* convertMatrixToHistogram<TMatrixT<double> >(TMatrixT<double>* matrix, const TString& name);
  template TH2* convertMatrixToHistogram<TMatrixTSym<float > >(TMatrixTSym<float >* matrix, const TString& name);
  template TH2* convertMatrixToHistogram<TMatrixTSym<double> >(TMatrixTSym<double>* matrix, const TString& name);
}
