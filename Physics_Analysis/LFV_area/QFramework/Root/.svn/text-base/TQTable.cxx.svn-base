...
void TQTable::merge(TQTable* other){
  ...
  int nElements = 0;
  for(size_t i=0; i<other->nrows - 1; i++){
...
int TQTable::appendLines(const TQTable& other, int startAt, bool ignoreHlines){
  ...
  if(!ignoreHlines){
    if(other.hlines.size() > 0){
      if(this->nrows == 0){
        ...
        for(size_t i=this->hlines.size(); i<=this->nrows; i++){
          this->hlines.push_back(0);
        }
      }
    }
  ...
  int offset = oldrows * this->ncols;
  int nElements = 0;
  for(size_t i=0; i<other.nrows - startAt; i++){
    for(size_t j=0; j<other.ncols; j++){
  ...
  if(!ignoreHlines){
    for(size_t i=startAt+1; i< other.hlines.size(); i++){
...
bool TQTable::setVline(int col, int type){
  ...
  while((size_t)(col+1) > this->vlines.size()) this->vlines.push_back(0);
...
void TQTable::setColAlign(size_t col, TString align){
...
TString TQTable::getColAlign(size_t col){
...
TString TQTable::getColAlignHTML(size_t col){
  // retrieve the horizontal alignment of the given column
...
bool TQTable::setHline(int row, int type){
  ...
  while((size_t)(row+1) > this->hlines.size()) this->hlines.push_back(0);
...
void TQTable::readCSV(std::istream* input, const TString& sep, const TString& leftquote, const TString& rightquote){
  ...
  if(this->ncols == 0){
    this->ncols = TQStringUtils::countText(linebuffer, sep)+1;
  } else getline(*input, linebuffer);
  size_t i = this->nrows;
  this->nrows+= 256;
  this->data = (TQTaggable**)realloc(this->data, this->ncols*this->nrows*sizeof(TQTaggable*));
  size_t pos=0;
  size_t nextpos=0;
  bool eof = false;
  bool empty;
...
void TQTable::readTSV(std::istream* input, const TString& seps, int nsepmin){
  ...
  size_t i = this->nrows;
  this->nrows+= 256;
  if(this->ncols == 0) this->ncols = 256;
  this->data = (TQTaggable**)realloc(this->data,this->ncols * this->nrows * sizeof(TQTaggable*));
  size_t pos=0;
  bool eof = false;
  bool empty;
...
void TQTable::readHTML(std::istream* input){
  ...
  size_t pos = TQStringUtils::find(content,"<tr>");
  size_t nextpos= TQStringUtils::find(content,"</tr>", pos);
  ...
  this->data = (TQTaggable**)realloc(this->data, this->ncols*this->nrows*sizeof(TQTaggable*));
  size_t lineend = 1;
  while(pos < (size_t)content.Length()){
    if(i == this->nrows){
      this->nrows *= 2;
      this->data = (TQTaggable**)realloc(this->data, this->ncols*this->nrows*sizeof(TQTaggable*));
    }
    ...
    i++;
    if(lineend < (size_t)content.Length()) pos = TQStringUtils::find(content,"<tr>", lineend);
    else break;
  }
  this->nrows = i;
  this->data = (TQTaggable**)realloc(this->data, this->ncols*this->nrows*sizeof(TQTaggable*));
...
void TQTable::readLaTeX(std::istream* input){
  ...
  for(size_t i=0; i<tableHeads.size(); i++){
    pos = std::min(pos, (size_t)TQStringUtils::find(content,"\\begin{"+tableHeads[i]+"}"));
  }
  pos = TQStringUtils::find(content,"}", pos);
  pos = TQStringUtils::find(content,"{", pos)+1;
  size_t nextpos = findParenthesisMatch(content, pos, "{", "}");
  std::vector<TString> buffer = TQStringUtils::split(content(pos, nextpos-pos), " |\t", "{", "}");
  std::string bufstr;
  if(this->ncols == 0) this->ncols = buffer.size();
  size_t i = this->nrows;
  this->nrows += 256;
  this->data = (TQTaggable**)realloc(this->data, this->ncols*this->nrows*sizeof(TQTaggable*));
  pos = nextpos;
  ...
  while(pos < (size_t)content.Length()){
    ...
    for(size_t j=0; j<std::min((size_t)(this->ncols), buffer.size()); j++){
      TQTaggable* newEntry = new TQTaggable();
...
bool TQTable::printCSV(std::ostream* output, TQTaggable tags){
  ...
  for(size_t i=0; i<this->nrows; i++){
    for(size_t j=0; j<this->ncols-1; j++){
...
bool TQTable::printPlain(std::ostream* output, TQTaggable tags){
  ...
  for(size_t i=0; i<this->hlines.size(); i++){
  ...
  for(size_t i=0; i<this->nrows; i++){
    ...
    for(size_t j=0; j+1<this->ncols; j++){
...
int TQTable::getEntryInteger(size_t i, size_t j){
...
TString TQTable::getEntry(size_t i, size_t j, TString format){
  // retrieve the table entry at row i and column j
  // in the given format
  format.ToLower();
...
TString TQTable::getEntryPlain(size_t i, size_t j, bool allowUnicode){
  // retrieve the table entry at row i and column j
...
TString TQTable::getEntryVerbatim(size_t i, size_t j){
  // retrieve the table entry at row i and column j
  // as verbatim text
  TQTaggable* entry = this->getEntryInternal(i,j);
...
TString TQTable::getEntryUnicode(size_t i, size_t j){
  // retrieve the table entry at row i and column j
  // in Unicode-Format
  return this->getEntryPlain(i,j,true);
}

TString TQTable::getEntryASCII(size_t i, size_t j){
  // retrieve the table entry at row i and column j
  // in ASCII-Format
  return this->getEntryPlain(i,j,false);
}

TString TQTable::getEntryHTML(size_t i, size_t j){
  // retrieve the table entry at row i and column j
  // in HTML format
  TQTaggable* entry = this->getEntryInternal(i,j);
...
TString TQTable::getEntryLaTeX(size_t i, size_t j){
  // retrieve the table entry at row i and column j
  // in LaTeX format
  TQTaggable* entry = this->getEntryInternal(i,j);
...
bool TQTable::setEntry(size_t i, size_t j, const TString& content, const TString& format){
...
bool TQTable::setEntry(size_t i, size_t j, const char* content, const TString& format){
  // set the table entry at row i and column j to the given content
  // input format can be specified with last argument and will be guessed automatically if left empty
  // contents will automatically be converted into all supported output formats
  // if you do not want this or want to edit individual representations
...
bool TQTable::setEntryValue(size_t i, size_t j, double content){
...
double TQTable::getEntryValue(size_t i, size_t j, double defaultval){
...
bool TQTable::setEntryValueAndUncertainty(size_t i, size_t j, double value, double Uncertainty){
  // set the table entry at row i and column j to the given numerical content
  TQTaggable* entry = this->getEntryInternal(i,j,true);
  if(!entry) return false;
...
bool TQTable::setEntryValue(size_t i, size_t j, int content){
  // set the table entry at row i and column j to the given numerical content
  TQTaggable* entry = this->getEntryInternal(i,j,true);
  if(!entry) return false;
...
bool TQTable::expand(size_t i, size_t j){
  ...
  size_t rows = std::max(i, (size_t)this->nrows);
  size_t cols = std::max(j,(size_t)this->ncols);
  if(cols > this->ncols){
    ...
    for(size_t i=0; i<this->nrows; i++){
      for(size_t j=0; j<this->ncols; j++){
        this->data[i*cols + j] = old[i*this->ncols + j];
      }
    }
    free(old);
    this->nrows = rows;
    ...
    this->data = (TQTaggable**)realloc(this->data,rows*this->ncols*sizeof(TQTaggable*));
    if(!data) return false;
    for(size_t i=this->nrows; i<rows; i++){
      for(size_t j=0; j<this->ncols; j++){
        this->data[i*cols + j] = NULL;
      }
    }
    this->nrows = rows;
    this->nfields = this->nrows * this->ncols;
...
void TQTable::clearRow(size_t row){
...
void TQTable::clearCol(size_t col){
...
bool TQTable::hasEntry(size_t i, size_t j){
...
void TQTable::removeEntry(size_t i,size_t j){
...
bool TQTable::setProperty(size_t i, size_t j, const TString& key, const TString& value){
  // set a property/style tag for entry at row i and column j 
  TQTaggable* entry = this->getEntryInternal(i,j,true);
  if(entry) return entry->setTagString(key,value);
  return false;
}
bool TQTable::setProperty(size_t i, size_t j, const TString& key, const char* value){
  // set a property/style tag for entry at row i and column j 
  TQTaggable* entry = this->getEntryInternal(i,j,true);
  if(entry) return entry->setTagString(key,value);
  return false;
}
bool TQTable::setProperty(size_t i, size_t j, const TString& key, double value){
  // set a property/style tag for entry at row i and column j 
  TQTaggable* entry = this->getEntryInternal(i,j,true);
  if(entry) return entry->setTagDouble(key,value);
  return false;
}
bool TQTable::setProperty(size_t i, size_t j, const TString& key, int value){
  TQTaggable* entry = this->getEntryInternal(i,j,true);
  if(entry) return entry->setTagInteger(key,value);
  return false;
}
void TQTable::setColProperty(size_t j, const TString& key, const TString& value){
  // set a property/style tag for all entries in column j 
  for(size_t i=0; i<this->nrows; i++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setColProperty(size_t j, const TString& key, const char* value){
  // set a property/style tag for all entries in column j 
  for(size_t i=0; i<this->nrows; i++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setColProperty(size_t j, const TString& key, double value){
  // set a property/style tag for all entries in column j 
  for(size_t i=0; i<this->nrows; i++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setColProperty(size_t j, const TString& key, int value){
  // set a property/style tag for all entries in column j 
  for(size_t i=0; i<this->nrows; i++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setRowProperty(size_t i, const TString& key, const TString& value){
  // set a property/style tag for all entries in row i 
  for(size_t j=0; j<this->ncols; j++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setRowProperty(size_t i, const TString& key, const char* value){
  // set a property/style tag for all entries in row i 
  for(size_t j=0; j<this->ncols; j++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setRowProperty(size_t i, const TString& key, double value){
  // set a property/style tag for all entries in row i 
  for(size_t j=0; j<this->ncols; j++){
    this->setProperty(i,j,key,value);
  }
}
void TQTable::setRowProperty(size_t i, const TString& key, int value){
  // set a property/style tag for all entries in row i 
  for(size_t j=0; j<this->ncols; j++){
    this->setProperty(i,j,key,value);
  }
...
TString TQTable::getRowAsCSV(int row, const TString& sep){
  ...
  TString retval;
  if((size_t)row > this->nrows) return retval;
...
void TQTable::setFromTList(TList& l){
  ...
  for(size_t i=0; i<(size_t)l.GetEntries(); i++){
...
std::map<TString,TString> TQTable::getMap(size_t keyidx, size_t validx, const TString& keyformat, const TString& valformat, bool skipfirstline){
  ...
  for(size_t i=skipfirstline; i<this->nrows; i++){
...
int TQTable::readColumn(TQTable* other, int col, int thismatchcol,int othermatchcol){
  // read a column from another table
  size_t newcol = this->getNcols();
  int set = 0;
  for(size_t row=1; row<(size_t)(other->getNrows()); ++row){
