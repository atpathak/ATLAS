...
TEnv* TQConfigReader::getEnv(){
  ...
  std::set<TString> _files_included;
 
  bool add_more = false;
  do {
    TString s;
    if (!TQStringUtils::isEmpty(_includePaths)) {//this is the first loop, we might want to override the config.Include parameter
      s = _includePaths;
    } else {
      s = _env->GetValue("config.Include", "") ;
    }
    add_more = false;
    //TString s(_env->GetValue("config.Include", "")) ;
    //std::cout<<"Got config.Include tag, it reads: '"<<s.Data()<<"'"<<std::endl;
    std::auto_ptr<TObjArray> includes(s.Tokenize(" ,\t"));
    TIter iter(includes.get());
    while(TObject *p = iter.Next()) {
      TString filename(((TObjString*)p)->GetString().Data());
      ...
      INFO("including configuration file '%s'",filename.Data());
