...
TList * TQFolder::getListOfFolders(const TString& path_, TQFolder * template_, bool toplevelOnly) {
  ...
  return this->getListOfFolders(path_,template_ ? template_->IsA() : TQFolder::Class());
}
...
TQFolder * TQFolder::getBase(int depth) {
...
bool TQFolder::importFromTextPrivate(TString input, int &nNewlines, TString &errFile, TString &errMsg) {
  ...
  while (!input.IsNull()) {
    ...
    if (input.BeginsWith("#")) {
      ...
      bool expectNestedBlock = isCmdIgnore;
...
bool TQFolder::merge(TQFolder* other, bool sumElements){
  ...
  if(this->Class() == TQFolder::Class()){
    return this->mergeAsFolder(other,sumElements);
  } else {
...
bool TQFolder::mergeAsFolder(TQFolder* other, bool sumElements){
  ...
  while(itr.hasNext()){
    ...
    if(thisF){
      thisF->mergeAsFolder(f,sumElements);
    } else {
      f->detachFromBase();
      this->addObject(f);
    }
  ...
  // merge objects
  this->mergeObjects(other,sumElements);
  // return
  return true;
}
...
void TQFolder::mergeObjects(TQFolder* other, bool sumElements){
  ...
  while(itr.hasNext()){
    ...
    if(thisObj){
      TH1* hist = dynamic_cast<TH1*>(obj);
      TH1* thisHist = dynamic_cast<TH1*>(thisObj);
      if(hist && TQHistogramUtils::checkConsistency(hist,thisHist)){
        if (sumElements) thisHist->Add(hist);
        continue;
      }
      THnBase* ndimHist = dynamic_cast<THnBase*>(obj);
      THnBase* thisndimHist = dynamic_cast<THnBase*>(thisObj);
      if(ndimHist && TQTHnBaseUtils::checkConsistency(ndimHist,thisndimHist)){
        if (sumElements) thisndimHist->Add(ndimHist);
        continue;
      }
      TQCounter* counter = dynamic_cast<TQCounter*>(obj);
      TQCounter* thisCounter = dynamic_cast<TQCounter*>(thisObj);
      if(counter && thisCounter){
        if (sumElements) thisCounter->add(counter);
        continue;
      }
      TObjString* str = dynamic_cast<TObjString*>(obj);
      TObjString* thisStr = dynamic_cast<TObjString*>(thisObj);
      if(str && thisStr){
      ...
      TQTable* tbl = dynamic_cast<TQTable*>(obj);
      TQTable* thisTbl = dynamic_cast<TQTable*>(thisObj);
      if(tbl && thisTbl){
        if(sumElements) thisTbl->merge(tbl);
        continue;
      }
...
int TQFolder::replaceInFolderTags(TQTaggable& params, const TString& path, const TString& tagFilter, TClass* typeFilter ){
  ...
  if (!targetList) {
    ERRORclass("No matching folders found for pattern '%s'",path.Data());
    return -1;
  }
  TQFolderIterator itr(targetList);
  while (itr.hasNext()) {
