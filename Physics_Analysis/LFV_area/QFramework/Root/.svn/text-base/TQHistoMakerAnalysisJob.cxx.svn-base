...
void TQHistoMakerAnalysisJob::printBookingTeX(const TString& moretext) {
  ...
  std::cout << "\\multicolumn{2}{l}{\\bfseries Booked Histograms}" << " & \\multicolumn{2}{l}{" << moretext  << "}\\tabularnewline" << std::endl;
  ...
  for (unsigned int i = 0; i < fHistograms.size(); ++i) {
    ...
    case -2:
      if (this->fFillSynchronized[i]) {
        ...
        //at this point we should be safe calling gNevaluations
    ...
    case 1:
      {
        ...
        //        try {
          ...
          for (int a=0; a<fObservables[i][0]->getNevaluations(); a++) {
            ...
            DEBUGclass("filling histogram '%s' from '%s' with value %f",fHistograms[i]->GetName(),fObservables[i][0]->getActiveExpression().Data(),val);
            fHistograms[i]->Fill(val,this->fFillRaw[i]? 1. : weight*fSample->getNormalisation()); 
          }
