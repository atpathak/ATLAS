...
void TQSampleInitializerBase::readDirectory(const TString& path, int maxdepth){
  ...
  this->fPaths = TQFolder::copyDirectoryStructure(path, maxdepth);
...
bool TQSampleInitializerBase::readInputFilesList(const TString& listpath, bool verbose){
  ...
  for (auto path : fileList){
	...
	TString basepath(path(0,pathstart));
	this->fPaths->setTagString("basepath",basepath);
      }
      fnew = this->fPaths->getFolder(pathToFind+"+!");      
      if(verbose){ VERBOSEclass("adding file '%s' in '%s'",name.Data(),pathToFind.Data()); };
    }    
    if(fnew) {
