...
bool TQCut::addAnalysisJob(TQAnalysisJob * newJob_, const TString& cuts_) {
  ...
  /* wildcard "*" adds the job to every cut */
  if (cuts.CompareTo("*") == 0) {
    TQIterator itr(this->getListOfCuts(),true);
    while(itr.hasNext()){
      TQCut* cut = dynamic_cast<TQCut*>(itr.readNext());
      if(cut) cut->addAnalysisJob(newJob_);
    }
    ...
    while(itr.hasNext()){
      ...
      /* check for wildcard "*" at the end of the cut name */
      TString cutOpt = "";
      if(TQStringUtils::removeTrailing(cutName,"*") > 0)
        cutOpt = "*";
      TQStringUtils::removeTrailing(cutName,"/");
 
      /* try to find the cut */
      TQCut * cut = this->getCut(cutName);
      if(!cut){
      ...
      if (!cut->addAnalysisJob(newJob_, cutOpt)) success = false;
    } 
  }
...
bool TQCut::initialize(TQSample * sample) {
  ...
  if (!this->fSkipAnalysisJobs && !this->fSkipAnalysisJobsGlobal) {
    DEBUGclass("iterating over analysis jobs");
    TQAnalysisJobIterator itr(fAnalysisJobs);
    while(itr.hasNext()){
      TQAnalysisJob* job = itr.readNext();
