...
TGraph* TQLibrary::getMemoryGraph() {
  ...
  if (dTimestamps.size() != dMemory.size() || dMemory.size()==0) return NULL;
  return new TGraph(dTimestamps.size(),&dTimestamps[0],&dMemory[0]);
}
...
TString TQLibrary::findLibrary(const TString& filename){
  ...
  TString retval = "";
  TList* paths = TQUtils::execute("find $(g++ --print-search-dirs | grep libraries | sed -e 's/^[ ]*libraries:[ ]*=//' -e 's/:/ /g') -name 'libxml2.*so' 2>/dev/null -print -quit",255);
  paths->SetOwner(true);
