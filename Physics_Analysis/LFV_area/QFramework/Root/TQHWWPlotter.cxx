...
TObjArray* TQHWWPlotter::collectHistograms(TQTaggable& tags){
  ...
  if(tags.hasTag("errors.showSys")){
    // the "new" way
    if(!this->includeSystematics(tags)){
      WARNclass("unable to retrieve systematics!");
    }
  }
  ...
  if(hTotalBkg){
    ...
    while(itr.hasNext()){
      ...
      if(tags.getTagBoolDefault ("style.showChi2",false)){
        ...
        Double_t chi2 = 0;
        Int_t ndf = 0, igood = 0;
        double prob = hTotalBkg->Chi2TestX(h_data,chi2,ndf,igood);
      ...
      if(tags.getTagBoolDefault ("style.showChi2P",false)){
        double p_value = h_data->Chi2Test(hTotalBkg,"UW");
