...
bool TQPlotter::includeSystematics(TQTaggable& tags){
  ...
  if(!hSysOrig){
    ...
    hSys->Scale(sys);
    if(verbose) VERBOSEclass("unable to retrieve shape systematic histogram for '%s/%s', using normalization systematic of ~%f instead",cutname.Data(),histName.Data(),sys);
    return true;
  } else {
    if(verbose) VERBOSEclass("successfully retrieved shape systematic histogram '%s'",histName.Data());
    if(!TQHistogramUtils::checkConsistency(hTotalBkg,hSysOrig,verbose)){
      if(verbose) VERBOSEclass("master and systematics histograms are inconsistent!");
      return false;
    }
