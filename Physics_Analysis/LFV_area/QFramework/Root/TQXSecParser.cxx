...
int TQXSecParser::addAllSamples(bool requireEnabled, bool requirePath){
  ...
  if(!this->getTagBoolDefault(".hasMapping",false)){
    if(this->readMappingFromColumn("*path*") < 1){
      ERRORclass("refusing to add samples - no mapping specified, no adquate column found");
      return -1;
    }
  }
  TString treeName;
  for(size_t i=1; i<this->nrows; i++){
    ...
    while(itr.hasNext()){
      ...
      if(xSecCol > 0){
        double xsec = atof(this->getEntryPlain(i,xSecCol).Data());
        s->setTagDouble(".xsp.xSection",TQXSecParser::convertUnit(xsec,unit,TQXSecParser::Units::picobarn));
      }
      if(filterEffCol > 0){
        double filtereff = atof(this->getEntryPlain(i,filterEffCol).Data());
        s->setTagDouble(".xsp.filterEfficiency",filtereff);
      }
      if(kFactorCol > 0){
        double kfactor = atof(this->getEntryPlain(i,kFactorCol).Data());
      ...
      if(sowCol > 0){
        ...
        if(!sumOfWeightsInfo.IsNull()){
          double sumOfWeightsPerEvent = atof(sumOfWeightsInfo);
