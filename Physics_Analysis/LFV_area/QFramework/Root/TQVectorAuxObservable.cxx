...
const TString& TQVectorAuxObservable::getExpression() const {
  ...
  this->fFullExpression = this->fSubObservable? this->fSubObservable->getExpression() : this->fExpression;
...
TString TQVectorAuxObservable::getActiveExpression() const {
  ...
  TString aExpr = this->fSubObservable? this->fSubObservable->getActiveExpression() : this->fExpression;
...
bool TQVectorAuxObservable::initializeSelf(){
  ...
  if (this->fOperation == AT) {
    ...
    if (expressions.size()==2) {
      this->fExpression = expressions[0];
  ...
  this->fSubObservable = TQObservable::getObservable(this->fExpression,this->fSample);
  if (!this->fSubObservable) {
    ERRORclass("Failed to obtain sub-observable!");
    throw std::runtime_error(TString::Format("Failed to obtain observable from expression '%s'",this->fExpression.Data()).Data());
  }
  DEBUGclass("Going to initialize sub-observable");
  if (!this->fSubObservable->initialize(this->fSample)) {
    throw std::runtime_error(TString::Format("Failed to initialize observable with name '%s' created from expression '%s' for sample '%s'",this->fSubObservable->GetName(), this->fExpression.Data(), this->fSample->getPath().Data()).Data());
  }
