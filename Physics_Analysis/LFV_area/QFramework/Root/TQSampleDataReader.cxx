  ...
  while (pIter.hasNext() && nElements >= 0) {
    ...
    while(pIter2.hasNext() && nElements >= 0){
      ...
      while(eIter.hasNext() && nElements >= 0) {
        ...
        TList * baseScaleList = this->getBaseScaleFactors(sf, elementName, scaleScheme, 1);
        if (!baseScaleList) {
...
int TQSampleDataReader::sumElements( TList * list, TQCounterGrid * &countergrid, TQTaggable * options ) {
  // sum all elements in the list (TQPCA variant)
  int nElements = 0;
  if (!list) return -1;
  TQListIterator itr(list);
  while(itr.hasNext()){
    TList* l = itr.readNext();
    if(!l || l->GetSize() < 2) continue;
    TQCounterGrid* cg = dynamic_cast<TQCounterGrid*>(l->At(1));
    if(!cg) continue;
    double scale = 1.;
    for (int j=2; j<l->GetSize(); j++) {
    ...
    if (countergrid) {
      //ToDo: handle scale uncertainties properly!
      countergrid->add(cg,scale);
    } else {
      countergrid = new TQCounterGrid(*cg);
      countergrid->resetProjectionRanges();
      countergrid->scale(scale);
    }
    nElements++;
  }
  return nElements;
...
TQCounterGrid * TQSampleDataReader::getCounterGrid(const TString& path, const TString& name, const TString& options, TList * sfList) {
  // retrieve a countergrid (multi-dimensional histogram) path/name with the given options
  TQTaggable * tagsOptions = new TQTaggable(options);
  TQCounterGrid * grid = getCounterGrid(path, name, tagsOptions, sfList);
  delete tagsOptions;
  return grid;
}
...
TQCounterGrid * TQSampleDataReader::getCounterGrid(const TString& path, const TString& name, TQTaggable * options, TList * sfList) {
  // retrieve a countergrid (multi-dimensional histogram) path/name with the given options
  return dynamic_cast<TQCounterGrid*>(this->getElement<TQCounterGrid>(path, name, this->f_pathGridScan, options, sfList));
}
...
TList * TQSampleDataReader::getBaseScaleFactors(TQSampleFolder * sampleFolder, TString path, TString scaleScheme, int indent) {
...
TList * TQSampleDataReader::getListOfCounterGridNames(const TString& path, TList * sfList) {
 
  // the list to return
  TList * objectNames = 0;
 
  // add the object names to the list
  this->addObjectNames(path, f_pathGridScan, TQCounterGrid::Class(), objectNames, sfList, "");
