...
int TQSampleFolder::visitMe(TQSampleVisitor * visitor) {
...
int TQSampleFolder::visitSampleFolders(TQSampleVisitor * visitor, const TString& category) {
  ...
  // the corresponding sample folders are visited non-recursively.
  ...
  /* remember the the number of visits */
  int nVisits = 0;
  
  for(size_t i=0; i<categories.size(); ++i){
    
    // loop over all sample folders matching <category>
    TCollection* c = this->getListOfSampleFolders(categories[i]);
    if(!c) continue;
    ...
    if (itr.getCounter() == 0) {
      WARNclass("No matching sample folder for '%s' found", categories[i].Data());
    }
...
int TQSampleFolder::generalizeObjectsPrivate(TList * names, TClass* objClass, TQTaggable* options, const TString& subpath) {
  ...
  while (itr.hasNext()){
    ...
    /* the list os samples folders contributing to the element */
    ...
    if (folder) {
      ...
      folder->setTagBool(".generalize.visited",true);
...
bool TQSampleFolder::merge(TQSampleFolder* f, const TString& traceID, bool sumElements, bool verbose){
  ...
  if(this->Class() == TQSampleFolder::Class()){
    return this->mergeAsSampleFolder(f,traceID,sumElements,verbose);
  } else {
...
bool TQSampleFolder::mergeAsSampleFolder(TQSampleFolder* f, const TString& traceID, bool sumElements, bool verbose){
  // merge two sample folders
  if(!f) return false;
  ...
  while(itr.hasNext()){
    ...
    if(!thisObj){
      // if we don't have an object of that type, we don't need to add it
      ...
      if(otherSample){
        ...
        if(!thisSample){
          ...
          if(!otherSample->hasSubSamples() && !thisSample->hasSubSamples()){
            ...
            if(!thisSample->getTagInteger("."+traceID+".timestamp.machine",thisDate) || !otherSF->getTagInteger("."+traceID+".timestamp.machine",otherDate) || (otherDate > thisDate) ){
              if(verbose) VERBOSEclass("Sample with no sub-samples, merge '%s' with timestamps: this is %s, other is %s",otherSample->getPath().Data(),
              ...
              didMerge = thisSample->mergeAsFolder(otherSample,sumElements) || didMerge;
            } else {
              if(verbose) VERBOSEclass("Sample with no sub-samples '%s' skipped due to timestamps: this is %s, other is %s",otherSample->getPath().Data(),
                                       thisSample->getTagStringDefault("."+traceID+".timestamp.human","<unknown>").Data(),
                                       otherSample->getTagStringDefault("."+traceID+".timestamp.human","<unknown>").Data());
            }
          } else if(otherSample->hasSubSamples() && thisSample->hasSubSamples()){
            // both samples have sub samples
            // all we need to do is merge them (this method has no implementation yet)
            // thisSample->mergeSubSamples(otherSample,sumElements,verbose);
            ...
            didMerge = thisSample->mergeAsSampleFolder(otherSample,traceID,sumElements,verbose) || didMerge;
          } else {
        ...
        if(!thisSF){
          ...
          if(otherSF->getTagBoolDefault("."+traceID+".visited~",false)){
            ...
            if(thisSF->getTagBoolDefault("."+traceID+".visited~",false)){
              ...
              didMerge = thisSF->mergeAsSampleFolder(otherSF,traceID,sumElements,verbose) || didMerge;
            } else {
        ...
        if(!thisF){
          ...
          didMerge = (thisF->*&TQSampleFolder::mergeAsFolder)(otherF,sumElements) || didMerge;
          // what it actually does is just 
          // thisF->mergeAsFolder(otherF,sumElements)
          // for reference, see 
  ...
  this->mergeTags(f);
  this->mergeObjects(f,sumElements);
  return didMerge;
}
...
void TQSampleFolder::findFriends(TQSampleFolder* otherSF, bool forceUpdateSubfolder){
  //TQSample* thisSample = dynamic_cast<TQSample*>(this);
  //if (thisSample) thisSample->findFriendsInternal(otherSF,forceUpdateSubfolder); //call the child's method if we have an instance of TQSample
  //else this->findFriendsInternal(otherSF,forceUpdateSubfolder);
...
void TQSampleFolder::findFriendsInternal(TQSampleFolder* otherSF, bool forceUpdateSubfolder){
  ...
  itr.reset();
  TQSampleIterator subitr(this->getListOfSampleFolders("?"));
  while(itr.hasNext()){
    ...
    while(subitr.hasNext() && befriend) {
      ...
      thisSub->findFriends(otherSF, forceUpdateSubfolder);
      otherSub->findFriends(otherSF, forceUpdateSubfolder);
      befriend = befriend && thisSub->isFriend(otherSub);
    }
    subitr.reset();
    if (befriend) this->befriend(other);
  }
}
...
void TQSampleFolder::befriend(TQSampleFolder* other){
  ...
  if(!this->fFriends){
    this->fFriends = new TList();
  }
  if(this->fFriends->FindObject(other)){
    DEBUGclass("'%s' is already a friend to '%s'",other->getPath().Data(),this->getPath().Data());
  } else {
    this->fFriends->Add(other);
    other->befriend(this);
  }
}
...
TCollection* TQSampleFolder::getFriends(){
...
int TQSampleFolder::countFriends(){
  ...
  return this->fFriends->GetEntries();
}
