...
#ifdef DEBUGBUILD
#define TRY(code,message) try{ code } catch (const std::runtime_error& error) { throw std::runtime_error( (TQStringUtils::makePink(TString::Format("[%s, line %i] " , __FILE__, __LINE__ )) + TString(message) + TString(" Preceeding error: \n -> ")+TString(error.what())).Data() ); }
#else
...
public:
  ...
  static void recordMemory(); //adds a point (TQUitls::getCurrentTime(),TQUtils::getCurrentRSS()) to an internal TGraph. TQUtils::getCurrentRSS uses precompiler statements to provide a mostly platform independent retrieval of memory usage.
  static TGraph* getMemoryGraph();
