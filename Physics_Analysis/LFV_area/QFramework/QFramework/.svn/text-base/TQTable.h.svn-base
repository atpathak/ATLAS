...
public: 
  ...
  std::map<TString,TString> getMap(size_t keyidx, size_t validx, const TString& keyformat = "ascii", const TString& valformat = "ascii", bool skipfirstline = false);

  int getNcols();
  int getNrows();
  TString getColAlign(size_t col);
  TString getColAlignHTML(size_t col);
  ...
  int getEntryInteger(size_t i, size_t j);
  TString getEntry(size_t i, size_t j, TString format = "plain");
  TString getEntryPlain(size_t i, size_t j, bool allowUnicode = true);
  TString getEntryASCII(size_t i, size_t j);
  TString getEntryVerbatim(size_t i, size_t j);
  TString getEntryUnicode(size_t i, size_t j);
  TString getEntryLaTeX(size_t i, size_t j);
  TString getEntryHTML(size_t i, size_t j);
  bool setEntry(size_t i, size_t j, const TString& content, const TString& format = "");
  bool setEntry(size_t i, size_t j, const char* content, const TString& format = "");
  bool setEntryValue(size_t i, size_t j, int content);
  bool setEntryValue(size_t i, size_t j, double content);
  bool setEntryValueAndUncertainty(size_t i, size_t j, double value, double uncertainty);
  double getEntryValue(size_t i, size_t j, double defaultval);
  bool setProperty(size_t i, size_t j, const TString& key, const TString& value);
  bool setProperty(size_t i, size_t j, const TString& key, const char* value);
  bool setProperty(size_t i, size_t j, const TString& key, double value);
  bool setProperty(size_t i, size_t j, const TString& key, int value);
  void setColProperty(size_t j, const TString& key, const TString& value);
  void setColProperty(size_t j, const TString& key, const char* value);
  void setColProperty(size_t j, const TString& key, double value);
  void setColProperty(size_t j, const TString& key, int value);
  void setRowProperty(size_t i, const TString& key, const TString& value);
  void setRowProperty(size_t i, const TString& key, const char* value);
  void setRowProperty(size_t i, const TString& key, double value);
  void setRowProperty(size_t i, const TString& key, int value);
  void removeEntry(size_t i, size_t j);
  void clearRow(size_t row);
  void clearCol(size_t col);
 
  bool hasEntry(size_t i, size_t j);

  bool setVline(int col = 1, int type = 1);
  bool setHline(int row = 1, int type = 1);
  bool clearVlines();
  bool clearHlines();
  void setColAlign(size_t i, TString align);

  bool expand(size_t i, size_t j);
  bool shrink();
  int cleanup();
  int clear();
