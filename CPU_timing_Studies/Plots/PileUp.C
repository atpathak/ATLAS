...
void PileUp() {
  ...
  TLegend *leg  = new TLegend(0.175,0.475,0.525, 0.775, "","brNDC");
  //
  leg->SetBorderSize(0);
  leg->SetLineColor(0);
  leg->SetFillColor(0);
  ...
  for (int irun = 0; irun < 2; ++irun){
	...
	if(ialg==4){
	  ...
	  gre4->SetMarkerStyle(26);
	  gre4->SetMarkerSize(2.2);
	...
	if(ialg==8){
	  ...
	  gre8->SetMarkerStyle(25);
	  gre8->SetMarkerSize(2.2);
  ...
  leg->AddEntry(gre8, "Total ID Run 2 Reconstruction", "p");
  leg->AddEntry(gr8,  "Total ITk Reconstruction", "lp");
  ...
  //leg->AddEntry(gre3, "Pattern recognition (Run 2)", "p");
  //leg->AddEntry(gr3,  "Pattern recognition (ITk)", "lp");
  
  leg->AddEntry(gre3, "Si Track Finding (Run 2)", "p");
  leg->AddEntry(gr3,  "Si Track Finding (ITk)", "lp");

  leg->AddEntry(gre4, "Ambiguity Resolution (Run 2)", "p");
  leg->AddEntry(gr4, "Ambiguity Resolution (ITk)", "lp");
...
*/
  ...
  //leg->AddEntry(gr6,  "Particle creation (ITk)", "lp");
      ...
      mg->GetYaxis()->SetTitle("HS06 #times seconds per Event");
      leg->Draw();
      //      leg2->Draw();
      c1->Modified();
      //mg->GetYaxis()->SetRangeUser(.00005,2000);
      mg->GetYaxis()->SetRangeUser(-10,310);
    ATLASLabelOld(0.2,.88,true,1);
      c1->Update();
      c1->SaveAs("Timing_ITk_Run2.pdf");
      c1->SaveAs("Timing_ITk_Run2.eps");
      c1->SaveAs("Timing_ITk_Run2.png");
