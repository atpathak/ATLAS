...
if [ $rtag = "9871" ] ; then
    ...
    input="SLHCtest_1.RDO.pool.root,SLHCtest_2.RDO.pool.root,SLHCtest_3.RDO.pool.root,SLHCtest_4.RDO.pool.root,SLHCtest_5.RDO.pool.root,SLHCtest_6.RDO.pool.root,SLHCtest_7.RDO.pool.root,SLHCtest_8.RDO.pool.root,SLHCtest_9.RDO.pool.root,SLHCtest_10.RDO.pool.root,SLHCtest_11.RDO.pool.root,SLHCtest_12.RDO.pool.root,SLHCtest_13.RDO.pool.root,SLHCtest_14.RDO.pool.root,SLHCtest_15.RDO.pool.root,SLHCtest_16.RDO.pool.root,SLHCtest_17.RDO.pool.root,SLHCtest_18.RDO.pool.root,SLHCtest_19.RDO.pool.root,SLHCtest_20.RDO.pool.root,SLHCtest_21.RDO.pool.root,SLHCtest_22.RDO.pool.root,SLHCtest_23.RDO.pool.root,SLHCtest_24.RDO.pool.root,SLHCtest_25.RDO.pool.root,SLHCtest_26.RDO.pool.root,SLHCtest_27.RDO.pool.root,SLHCtest_28.RDO.pool.root,SLHCtest_29.RDO.pool.root,SLHCtest_30.RDO.pool.root,SLHCtest_31.RDO.pool.root,SLHCtest_32.RDO.pool.root,SLHCtest_33.RDO.pool.root,SLHCtest_34.RDO.pool.root,SLHCtest_35.RDO.pool.root,SLHCtest_36.RDO.pool.root,SLHCtest_37.RDO.pool.root,SLHCtest_38.RDO.pool.root,SLHCtest_39.RDO.pool.root,SLHCtest_40.RDO.pool.root"
elif [ $rtag = "9875" ] ; then
    ami="r9875"
    AtProd="9"
    ...
    #input="SLHCtest_1.RDO.pool.root,SLHCtest_2.RDO.pool.root,SLHCtest_3.RDO.pool.root,SLHCtest_4.RDO.pool.root,SLHCtest_5.RDO.pool.root,SLHCtest_6.RDO.pool.root,SLHCtest_7.RDO.pool.root,SLHCtest_8.RDO.pool.root"
elif [ $rtag = "7725" ] ; then
    ami="r7725"
    AtProd="1"
...
if [ ${THIS_N} = "-1" -o ${THIS_N} = "${n}" ] ; then
 ...
 #skipevt=`echo $n | awk '{print 1000+($1-2)*50}'`
 #sleep=`echo $n | awk '{print 30*($1-1)}'`
 echo $rtag $n $skipevt
 rm -rf    VF_ITK_9Oct2017_r${rtag}/RunArea_${m}/Reconstr_${n}
 mkdir -vp VF_ITK_9Oct2017_r${rtag}/RunArea_${m}/Reconstr_${n}
 cd        VF_ITK_9Oct2017_r${rtag}/RunArea_${m}/Reconstr_${n}
 if [ $rtag = "9871" ] ; then
   cat > reco.todo <<EOF
Reco_tf.py \\
--inputRDOFile ${input} \\
...
--preInclude switch_off_trigger.py \\
...
EOF
 ...
 if [ $rtag = "9871" ] ; then
  ...
  /bin/ls -l /data/apathak/INPUT_r9871/mc15_14TeV/*root* | awk '{print "ln -s "$NF" SLHCtest_"NR+20".RDO.pool.root"}' | sh
 elif [ $rtag = "9875" ] ; then
  ...
  #/bin/ls -l /data/apathak/INPUT_r9877/mc15_14TeV/*root* | awk '{print "ln -s "$NF" SLHCtest_"NR+4".RDO.pool.root"}' | sh
 elif [ $rtag = "7725" ] ; then
  ...
  #/bin/ls -l /data/apathak/INPUT_r7725/mc15_13TeV_r7725/*root* | awk '{print "ln -s "$NF" SLHCtest_"NR+2".RDO.pool.root"}' | sh
 fi
if [ $rtag = "9877" ] ; then
cat > todo <<EOF
#!/bin/bash
...
sleep ${sleep}
cd /data/apathak/VF_ITK_9Oct2017_r${rtag}
...
cd /data/apathak/VF_ITK_9Oct2017_r${rtag}/RunArea_${m}/Reconstr_${n}
rm -f reco.log
time ./reco.todo 2>&1 > reco.log
EOF
elif [ $rtag = "7725" ] ; then
cat > switch_off_trigger.py <<EOF
# print rec
rec.doTrigger = False
EOF
cat > todo <<EOF
...
shopt -s expand_aliases
# directly go to directory to launch jobs so that multiple instances of asetup can be run simultaneously without interfereing with each other
cd /data/apathak/VF_ITK_11Oct2017_r${rtag}/RunArea_15/Reconstr_${n}
export ATLAS_LOCAL_ROOT_BASE=/cvmfs/atlas.cern.ch/repo/ATLASLocalRootBase
source /cvmfs/atlas.cern.ch/repo/ATLASLocalRootBase/user/atlasLocalSetup.sh
